...
class GGUFLinear(nn.Module):
    ...
    def forward(self, x):
        global _GPU_DEQUANT_OK
        if _CITY_DEQUANT is not None and _GPU_DEQUANT_OK and x.device.type == "cuda":
            try:
                data = torch.from_numpy(np.asarray(self._qweight)).to(x.device, non_blocking=True)
                w = _CITY_DEQUANT(data, QT(self.qtype_value), tuple(self.weight_shape), dtype=x.dtype)
                ...
                _GPU_DEQUANT_OK = False
                print(f"[Rebels JE] GPU dequant failed once ({type(e).__name__}: {e}); "
                      f"falling back to CPU numpy dequant for the rest of the run.", flush=True)
...
def _materialize_meta(root, entries, consumed, dtype):
    ...
    print(f"[Rebels JE] materialized {fixed} meta tensors from GGUF.", flush=True)
    if missing:
        raise RuntimeError(
