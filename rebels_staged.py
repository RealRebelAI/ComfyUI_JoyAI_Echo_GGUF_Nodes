...
class RebelsJE_StagedPipeline:
    ...
    def run(self, config_source, dit_gguf, gemma_path, gemma_format, connector_path,
        ...
        # ============================================================ STAGE 1
        # CONDITIONING CACHE: the CPU Gemma encode takes 15+ minutes and only
        # depends on (gemma weights, connector, prompt). Same prompt again ->
        # reuse the cached embeddings and skip Stage 1 entirely. This is the
        # single biggest speed win for iterating on seeds/settings.
        cond_key = (gemma_path, gemma_format, connector_path, prompt)
        cond_cpu = _COND_CACHE.get(cond_key)
        if cond_cpu is not None:
            print("[StagedJE] Stage 1/3: cached conditioning hit -- skipping Gemma encode.", flush=True)
        else:
            ...
            _COND_CACHE[cond_key] = cond_cpu
            if len(_COND_CACHE) > 4:  # keep RAM bounded
        ...
        model_key = (dit_gguf, config_source, video_height, video_width,
        ...
        offloader = SequentialOffloader(generator, device, pin_memory=False) if sequential_offload else None
